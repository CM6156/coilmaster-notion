-- 사용자-담당자 연동 시스템 구축
-- 파일명: setup_user_manager_sync.sql
...
-- 10. 기본 데이터 입력 (예시)
INSERT INTO public.managers (name, email, created_at, updated_at) 
VALUES 
  ('김영업', 'sales@coilmaster.com', NOW(), NOW()),
  ('박개발', 'dev@coilmaster.com', NOW(), NOW()),
  ('이관리', 'admin@coilmaster.com', NOW(), NOW())
ON CONFLICT (email) DO NOTHING;

-- 11. 코멘트 추가
COMMENT ON TABLE public.managers IS '담당자/매니저 정보 테이블';
COMMENT ON VIEW public.user_manager_sync_status IS '사용자-담당자 연동 상태 조회 뷰';
...
-- 성공 메시지
SELECT '사용자-담당자 연동 시스템이 성공적으로 구축되었습니다!' as message; 